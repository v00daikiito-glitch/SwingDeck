...
└─ SubIndicators
    ...
    ├─ MACD { fast_ema_period, slow_ema_period, signal_ema_period, source, visible }
    ├─ RSI { period, visible }
    └─ BollingerBands { period, multiplier, ma_type, source, band, visible }
```

※表示設定。baseline の期間を、計算のたびにここから取り直さない。
※ MACD の `source` 初期値は `"close"`。
※ BollingerBands の `band` 初期値は `"both"`（チャート表示は常に上下両方の線を出すため。マスター118〜138行目のルールと同じ）。
※ VolumeMA は kind `"volume_ma"` に対応（詳細はパック2）。
