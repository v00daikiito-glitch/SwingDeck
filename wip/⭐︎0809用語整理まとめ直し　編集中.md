...
| `type:"baseline"` + `role` | 役割つき線 | `symbol_baselines`（数値1つへの決め方は U1） |
...
| baseline_json | TEXT | はい | 基準値1個ぶんのJSON。中身は §3-D。`output_kind` 列は置かない |
...
kind が `"deviation"` の場合: `params` は `{ left, right }`（形の詳細はパック2 2-4を参照）。`left` / `right` の分岐キーは `type`。

▼ metric_json の保存データ例（kind が `"deviation"`）
{
  "kind": "deviation",
  "timeframe": "daily",
  "params": {
...
│   │           args      … "deviation" のときは { left, right }
│   │                       ・output_kind は列と同じものを使うため、ここには書かない
│   │                       ・通常の基本の計算（left=今の値段、right=roleの値）は使わず、
│   │                        ここで指定した left・right を直接使う
...
類型B: 銘柄ごとの値段は columns に書かない（overrides[]のargsに指標や固定値を書くのは例外）。

role + output_kind: "deviation" の基本の計算（overridesが無い銘柄）:
  left  = 今の値段（current_price）
  right = symbol_baselines の、この role の値

overrides[]の例（role: entryの列で、ある銘柄だけMA50とMA200を直接比べたい場合）：ß

{
  "column_id": "col_entry_dev",
  "column_type": "baseline",
  "role": "entry",
  "output_kind": "deviation",
  "overrides": [
    {
      "symbol_id": "8267.T",
      "args": {
