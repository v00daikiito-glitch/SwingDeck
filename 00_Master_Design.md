...
※列の指標を左辺にする抽出（列ごとのチップ等）だけ、列の metric_id を参照してよい。
...
-----------------------------------------------------------------
指標定義とプレイリスト列（共通の骨・0802）
-----------------------------------------------------------------

【結論】
・列も、オレンジ抽出も、水色アミカケも、裏では同じ「指標定義」を使う
・ただし列そのものを user_filters（フィルター）としては作らない
・フィルター工房の本格UIは後回しでよい。指標定義＋列の形だけ先に固定する

【3つの層】
1. 指標定義 … 何を計算するか（例: 7日変化率、日足50MA乖離）
2. 列 … 指標定義をプレイリストに値として出す（順序・表示名）
3. 条件（フィルター）… 指標定義に比較を足す（例: 7日変化率 > 5%）
   - オレンジ抽出・水色アミカケはここを使う

【指標定義の共通形（案）】
・kind … change_rate / deviation / rsi / volume_ratio など
・timeframe … daily / weekly / monthly（必要なもの）
・params … 期間やターゲットなど（例: period=7, ma_type=SMA）
...
※既存の columns_json の indicator_name / params と同じ方向。名前は後で columns_json に合わせて一本化する。

【プレイリストの＋列（シンプルUI）】
  1. ＋を押す
  2. 種類を選ぶ（変化率 / MA乖離 / …）
  3. 必要な数字だけ選ぶ（例: 7・日）
  4. columns_json に列を1本追加する（＝指標定義をスロットに保存）
※このとき user_filters に行は作らない

【indicators への渡し方】
・表示中の列（＋必要なら抽出条件）から指標定義を集める
・注文書JSONに載せて calculate_all へ渡す（visible なものだけ計算）
・返ってきた値を列に表示／ソート／単純抽出に使う

【フィルター工房とのつなぎ（後でUIを足すとき）】
・工房の左辺・右辺は、同じ指標定義のカタログから選ぶ
・列で作った「7日変化率」は、工房では「使える指標」として見える（自動で完成条件にはしない）
・完成条件（比較つき）だけが user_filters に保存される

【やらない（今）】
・列追加のたびに user_filters を自動生成して「適応先：列」を持たせる
・工房UIの全部盛りを列より先に作る
