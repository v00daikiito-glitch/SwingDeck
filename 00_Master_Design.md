...
■ テーブル⑥：playlists
（プレイリストの基本情報のみ。「箱」の名前を定義する）

  - id (型: INTEGER) / 主キー・自動連番
  - user_id (型: TEXT) / ユーザー識別ID（デバイス同期用キー）
  - playlist_name (型: TEXT) / リスト名（例: "半導体監視", "高配当"）

■ テーブル⑦：playlist_symbols
（プレイリストの箱に入っている銘柄情報のみ。手動ソート用カラムは廃止）

  - id (型: INTEGER) / 主キー・自動連番
  - playlist_id (型: INTEGER) / 紐づくplaylistsテーブルのid
  - symbol_id (型: TEXT) / 銘柄コード
  - is_pinned (型: INTEGER) / 1=ピン留め（常に上部表示）, 0=通常


3-E. フロントエンドUI制御（Filter UI Builder）
     条件作成ビルダーのプルダウン連動と表示モード切替ルール
-----------------------------------------------------------------
〈仕様定義：UIレイアウト基本構造〉
