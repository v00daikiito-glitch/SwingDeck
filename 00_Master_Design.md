...
-----------------------------------------------------------------
6. 将来拡張メモ（未確定・後で詳細を決める）
-----------------------------------------------------------------
・Hold のほかに、銘柄ごとにチェックできる「特別プレイリスト」を増やす。名前は Hold / Setup（エントリー候補）/ Harvest（利確候補）
・`playlists.kind` を `'user'` / `'special'` の2種類にし、`'special'` の行だけ、もう1つ下の列（例: `special_key`）で `hold` / `setup` / `harvest` を区別する
・`special_key` の種類はDBの列を増やして管理せず、IndicatorCatalog と同じようにアプリのコード側の一覧（カタログ）で持つ。将来増やすときはカタログに1行足すだけでよい形にする
・チェックの操作は、どの画面から行っても、対象銘柄がまだ監視銘柄（user_watchlist）に入っていなければ自動で追加する。この挙動は Hold / Setup / Harvest で統一する（現状のHoldにある「プレイリスト画面では監視済みの銘柄しかチェックできない」という画面差は、このタイミングで解消する）
・Setup / Harvest は、`symbol_baselines` の role（entry / take_profit）とは名前をあえて分ける（プレイリスト＝分類の箱、baseline＝価格の線、という役割の違いを保つため）。ただし、チェックを入れた際に「対応する baseline（entryまたはtake_profit）を設定しますか？」と一度だけ確認する導線を用意し、両者をつなげる
・チェックボックスの表示場所は、プレイリスト画面・個別チャート画面の各銘柄行を基本とし、普段は1つのアイコンに折り込んでおく。Deck画面（分割チャート）への配置も候補とし、レイアウトはDeck画面の詳細設計時に決める
・DBの列名・既存のHoldルール表（3-Eの副作用ルール）の書き換えなど、詳細は未確定。3-E の本編を直すときに正式に決める
